...
class WeightedL1Loss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        weights = 1.0 + torch.tanh(absolute_errors / 100.0)  
...
class CoordinateNormalizeLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        normalized_errors = errors / (targets + 1e-8)  
...
common_feature_cols = sorted(list(all_labeled_cols_features.union(all_unlabeled_cols_features)))
...
X_train, X_val, y_train, y_val = train_test_split(
    X_temp, y_temp,
    test_size=0.333, random_state=42, shuffle=True  
)
...
class EnhancedTemporalConformerWithOptimizedBiLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.blocks:
            temporal_out = block(temporal_out)
        query = temporal_out.mean(dim=1, keepdim=True) 
...
full_true_ranges_m = true_ranges[sequence_length-1:] 
